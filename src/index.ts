...
class ServerlessLocalKinesis {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        setTimeout(async () => {
          ...
          this.serverlessLog(
            `🎊 Stream '${stream.StreamDescription.StreamName}' created with ${stream.StreamDescription.Shards.length} shard(s)`
          );
  ...
  private async watchEvents(streamName: string): Promise<void> {
    ...
    for (const name of keys(this.serverless.service.functions)) {
      ...
      const streamEvent = filter(
        serverlessFunction.events || [],
        (e) => "stream" in e
      );
